      ...
      wx.getUserInfo({
        ...
        fail: function (userError) {
          var error = new LoginError(
            constants.ERR_WX_GET_USER_INFO,
            "获取微信用户信息失败，请检查网络状态"
          );
        ...
        success: function (result) {
          ...
          if (data && data.code === 0 && data.data.skey) {
            ...
            if (res.userinfo) {
              ...
              var errorMessage =
                "登录失败(" + data.error + ")：" + (data.message || "未知错误");
              var noSessionError = new LoginError(
                constants.ERR_LOGIN_SESSION_NOT_RECEIVED,
                errorMessage
              );
